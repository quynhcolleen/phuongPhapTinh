...
class PPGauss():
    ...
    def maTranBanDau(self):
        ...
        for i, row in enumerate(self.mat):
            ...
            print(f"Hang {i}: {expr} = {rhs:.0f}")
              
    def hienThiMaTran(self):  
        col_labels = [f"x{j+1}" for j in range(self.N)] + ["= "]
    ...
    def Gauss(self):
        ...
        for k in range(self.N):
            i_max = max(range(k, self.N), key=lambda i: abs(self.mat[i][k]))
            if self.mat[i_max][k] == 0:
                print("Ma tran suy bien - khong giai duoc theo PP Gauss.")
                print("He vo nghiem." if self.mat[i_max][self.N] else "He vo so nghiem.")
                return
        ...
        for i in range(self.N - 1, -1, -1):
            x[i] = (self.mat[i][self.N] - sum(self.mat[i][j] * x[j] for j in range(i + 1, self.N))) / self.mat[i][i]
